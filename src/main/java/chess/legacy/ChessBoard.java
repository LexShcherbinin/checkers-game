...
import static chess.legacy.PieceHelper.checkEnemyKingOnBoard;
import static chess.legacy.PieceHelper.getAttackedFieldList;
import static chess.legacy.PieceHelper.getMoveList;
import static chess.legacy.PieceHelper.getPieceInSquare;
...
public class ChessBoard {
   ...
   */
  private Set<Square> attackedFields = new HashSet<>();
  ...
  public Set<Square> getAttackedFields() {
    ...
    if (pieceAfter.getName() == KING) {
      ...
      if (sideShiftHorizontal == 2) {
        if (pieceBefore.getColor() == WHITE) {
          IPieces rightRook = getPieceInSquare(this, new Square(0, 7), WHITE);

          pieces.remove(rightRook);
          pieces.add(new Rook(WHITE, new Square(0, 5)));

        } else {
          IPieces rightRook = getPieceInSquare(this, new Square(7, 7), BLACK);

          pieces.remove(rightRook);
          pieces.add(new Rook(BLACK, new Square(7, 5)));
        }

      } else if (sideShiftHorizontal == -2) {
        if (pieceBefore.getColor() == WHITE) {
          IPieces leftRook = getPieceInSquare(this, new Square(0, 0), WHITE);

          pieces.remove(leftRook);
          pieces.add(new Rook(WHITE, new Square(0, 3)));

        } else {
          IPieces leftRook = getPieceInSquare(this, new Square(7, 0), BLACK);

          pieces.remove(leftRook);
          pieces.add(new Rook(BLACK, new Square(7, 3)));
        }
      }
    }
